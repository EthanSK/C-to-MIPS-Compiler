...
void ILtoMIPS::convertInstr(ILinstr &instr, MIPSContext &context, std::vector<MIPSinstr> &mipsInstr)
{
    ...
    if (instr.opcode == "scu") { ILtoMIPS::scu(instr, context, mipsInstr); }
    if (instr.opcode == "scd") { ILtoMIPS::scd(instr, context, mipsInstr); }
    if (instr.opcode == "decl") { ILtoMIPS::decl(instr, context, mipsInstr); }
